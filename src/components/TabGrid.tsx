  ...
  const renderEditArea = () => {
    ...
    return (
        <div ref={editAreaRef} className="relative pl-14 mb-8">
            ...
            {/* Changed from overflow-hidden to overflow-visible to let Playhead dot show */}
                                    ...
                                    {Array.from({ length: instrument.stringCount }).map((_, strIdx) => {
                                        ...
                                        <div key={strIdx} className={`flex ${DIM.STRING_HEIGHT} relative last:border-0`}>
                                            ...
                                            {markersWithBeams.map((marker) => {
                                                ...
                                                // --- PLAYHEAD LOGIC ---
                                                // We need to calculate if the current playhead falls within this duration marker
                                                const isPlayingThisCell = currentColumnIndex >= marker.globalIdx && currentColumnIndex < marker.globalIdx + marker.span;
                                                
                                                // Calculate offset for smooth movement (optional, for now just snap to start of cell)
                                                // If you want smooth movement within a long note, we'd need fractional progress.
                                                // For now, just highlighting the active cell is enough.
                                                ...
                                                return (
                                                    <div key={globalColIdx} className="flex-1 relative border-r border-gray-800 last:border-0" style={{ width: `${widthPercent}%`, flex: `0 0 ${widthPercent}%` }}>
                                                         ...
                                                         {strIdx === 0 && isPlayingThisCell && (
                                                            <div className="absolute -top-3 left-0 w-3 h-3 bg-green-400 rounded-full z-[100] shadow-[0_0_10px_rgba(74,222,128,0.8)] animate-pulse pointer-events-none -ml-1.5"
                                                                 style={{ 
                                                                     // If we want it to move across the note: 
                                                                     left: `${((currentColumnIndex - marker.globalIdx) / marker.span) * 100}%` 
                                                                 }}
                                                            ></div>
                                                         )}
                                                         ...
                                                         {/* Playhead Line */}
                                                         {isPlayingThisCell && (
                                                            <div className="absolute top-0 bottom-0 left-0 w-[2px] bg-green-400/50 z-[100] pointer-events-none shadow-[0_0_8px_rgba(74,222,128,0.6)]"
                                                                 style={{ 
                                                                     left: `${((currentColumnIndex - marker.globalIdx) / marker.span) * 100}%` 
                                                                 }}
                                                            ></div>
